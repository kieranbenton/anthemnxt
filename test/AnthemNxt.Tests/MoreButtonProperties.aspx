...
<script runat="server">
    ...
    {
        // Add the client-side script to the page
        string script = @"
function button1_PreCallBack(button) {
    if (!confirm('Are you sure you want to perform this call back?')) {
        return false;
    }
    document.getElementById('" + this.button2.ClientID + @"').disabled = true;
}

function button1_PostCallBack(button) {
    document.getElementById('" + this.button2.ClientID + @"').disabled = false;
}
