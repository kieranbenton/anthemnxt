	...
	writer.RenderEndTag();
				...
				<li>
					<p>The <code>GetNow</code> function doesn't normally exist on our pages so we have 
						...
						with <code>AnthemNxt.Manager</code>:</p>
					<pre><code><strong>protected override void OnLoad(EventArgs e)
{
	base.OnLoad(e);

	AnthemNxt.Manager.Register(this);
...
&lt;/script&gt;");
				...
				<li>
					<p>Looking at that client-side function, we can see that it's trying to invoke a 
						...
						control class and make sure that it's public and has the <code>AnthemNxt.Method</code>
						attribute on it:</p>
					<pre><code><strong>[AnthemNxt.Method]
